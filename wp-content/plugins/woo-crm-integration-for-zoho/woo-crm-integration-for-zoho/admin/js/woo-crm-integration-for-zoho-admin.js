	...
	}; var n = {}; i.renderer =
	{
		...
		function (a) { a = d(a); return { column: a.data("dtr-index"), row: a.parent().data("dtr-index") } }); p.register("responsive.rebuild()", function () { return this.iterator("table", function (a) { a._responsive && a._responsive._classLogic() }) }); p.register("responsive.recalc()", function () { return this.iterator("table", function (a) { a._responsive && (a._responsive._resizeAuto(), a._responsive._resize()) }) }); p.register("responsive.hasHidden()", function () {
			var a = this.context[0]; return a._responsive ? -1 !== d.inArray(!1, a._responsive._responsiveOnlyHidden()) :
				!1
		}); p.registerPlural("columns().responsiveHidden()", "column().responsiveHidden()", function () { return this.iterator("column", function (a, b) { return a._responsive ? a._responsive._responsiveOnlyHidden()[b] : !1 }, 1) }); i.version = "1.0.0"; d.fn.dataTable.Responsive = i; d.fn.DataTable.Responsive = i; d(l).on("preInit.dt.dtr", function (a, b) {
...
function opentab(evt, wp_tab) {
	...
	evt.preventDefault();
	tabcontent = jQuery(".wciz-tabcontent");
	tablinks = jQuery(".wciz-tab-link");
	...
	if ('wciz-logs' == wp_tab && false == initiliased) {
		initiliased = true;
		enableDataTable();
		// get_zoho_to_woo_sync_log();
	}
	jQuery('.wciz-dashboard__header-button-wrap').css('display', 'flex');
	window.scrollTo(0, 0);
}
...
function opentabdefault() {
	...
	if ('wciz-logs' == wp_tab_hash && false == initiliased) {
		initiliased = true;
		enableDataTable();
		get_zoho_to_woo_sync_log();
	}
...
function showAuthForm(event) {
	event.preventDefault();
	jQuery('.wciz-intro__content').slideUp('400', function () {
		jQuery('.wciz-zoho-auth-wrap').slideDown();
	});
}
...
* @since      1.0.0
*
* @param uri    var           Tab selected ID.
* @param key    var           Tab selected ID.
* @param value  var           Tab selected ID.
*/
...
function enableDataTable() {
	...
	var ajax_url = jQuery('#wciz-logs').attr('ajax_url');
	var nonce = jQuery('#wciz-logs').attr('ajax_nonce');
	ajax_url = ajax_url + '?action=get_datatable_data&nonce=' + nonce;
	var log_dataTable = jQuery('#wciz-table').DataTable({
		responsive: true,
		autoWidth: false,
		columnDefs: [
			{ width: 'auto', targets: '_all' }
		],
		dom: 'lfBrtip', // Buttons extension.
		buttons: [
			{
				extend: 'collection',
				text: 'Export',
		...
		"ajax": {
			'url': ajax_url,
			'data': function (data) {
				var error = jQuery('#searchByError').val();
				// Append to data
				data.searchByError = error;
			}
		},
	...
	// Table Up Div Add
	jQuery(".dataTable").wrap("<div class='set-scroll'></div>");
	jQuery(".set-scroll").wrap("<div class='table-scroll'></div>");


	jQuery('#searchByError').change(function () {
		log_dataTable.draw();
	});
	jQuery('#wciz-woo-zoho-reload-log').on('click', function (e) {
		log_dataTable.ajax.reload();
	});
}
function get_zoho_to_woo_sync_log() {
	var action = 'get_zoho_to_woocommerce_sync_log_data';
	var ajax_url = jQuery('#wciz-logs').attr('ajax_url');
	var nonce = jQuery('#wciz-logs').attr('ajax_nonce');

	jQuery.post(ajax_url, { action, nonce }).done(function (response) {
		var data = JSON.parse(response);

		if (data.result == 'failure') {
			jQuery('.wciz-zoho-data-log').html(data.data);
		} else {
			let data = response;
			let Data_br = data.replace(/\\n/g, "<br>");
			let _data = Data_br.replaceAll('"', "");

			jQuery('.wciz-zoho-data-log').html(_data);

		}
	})
}
function addFeedBackLink(feed_back_link, feed_back_text) {
	if (jQuery('body').hasClass('post-type-wciz_crm_feed')) {
		jQuery('.page-title-action').after('<a class="page-title-action" href="' + feed_back_link + '">' + feed_back_text + '</a>');
	}
}
...
jQuery(document).ready(function ($) {
	...
	const disconnect = ajax_data.disconnect;
	const sync_product = ajax_data.sync_product;
	const prod_feed_id = ajax_data.prod_feed_id;
	const skip = ajax_data.skip;
	const action = ajax_data.ajax_action;
	const nonce = ajax_data.ajax_nonce;
	const connect = ajax_data.connect;
	const feed_settings = ajax_data.feed_form_settings;
	const feed_back_link = ajax_data.feed_back_link;
	const feed_back_text = ajax_data.feed_back_text;
	const delete_icon = ajax_data.delete_icon;
	const notice = ajax_data.notice;
	const boolean = ajax_data.boolean;
	const order_status = ajax_data.order_status;
	const mapped_zoho_status = ajax_data.mapped_zoho_status;
	const plugin_directory_url = ajax_data.plugin_directory_url;
	const date_range_note = ajax_data.date_range_note;
	const show_mapping = [
		{ 'crm_object': 'Purchase_Orders', 'crm_field': 'Status' },
		{ 'crm_object': 'Deals', 'crm_field': 'Stage' },
		{ 'crm_object': 'Sales_Orders', 'crm_field': 'Status' },
		{ 'crm_object': 'Quotes', 'crm_field': 'Quote_Stage' },
	]

	var crm_fields = '';
	var crm_selected_object = '';
	var default_mapping = '';
	var default_event = '';
	var default_primary = '';
	let feedFilters;
	addFeedBackLink(feed_back_link, feed_back_text);

	// Open default tab.
	opentabdefault('wciz-dashboard');

	maybeSetFieldMapping();

	$('#wciz-feeds-zoho-object').on('change', function (e) {
		var module = $(this).val();
		if (module == '-1') return;
		//var event = 'get_module_fields';
		var event = 'get_module_fields_for_mapping';
		crm_selected_object = module;
		var request = { action, event, nonce, module };
		jQuery.post(ajax_url, request).done(function (response) {
			response = JSON.parse(response);
			if (response.hasOwnProperty('crm_fields')) {
				crm_fields = response.crm_fields.fields;
				default_event = response.default_event;
				default_mapping = response.default_mapping_data;
				default_primary = response.default_primary;
				var feed_event = default_event;
				var event = 'changed_field_values_according_to_feed_event';
				var request = { action, event, nonce, feed_event };
				$.post(ajax_url, request).done(function (response) {
					response = JSON.parse(response);
					console.log(response);
					feed_settings.field_value = response.field_value;
					feed_settings.field_type = response.field_type;
					feed_settings.custom_value = response.custom_value;
					createFeedEventForm();
					createFieldsForm(crm_fields, module);
					createAddNewFieldForm(crm_fields);
					createStatusMappingForm();
					createProductDetailForm();
					createPrimaryFieldForm();
					createProductImageForm();
					createSyncVariableProductForm();
					createTaxRateForm();
					createUserTypeForm();
					makeselectboxsearchable();
				})
			}
		});
		...
		if ('-1' != crm_selected_object && '' != post_id && 'wciz_crm_feed' == post_type) {
			let event = 'create_feed_filters';
			jQuery.post(ajax_url, { action, event, crm_selected_object, nonce }).done(function (response) {
				response = JSON.parse(response);
				if (true == response.success) {

					// Create filter fields.
					jQuery('.wciz-initial-filter').find('.or-condition-filter').remove();
					objectFields = response.object_fields;
					feedFilters = response.filter;
					let html = createOrFilteredHtml();
					jQuery('.condition-or-btn').before(html);
					jQuery('.condition-or-btn').attr('data-next-or-index', 2);
					jQuery('#wciz_zoho_feeds_condition_meta_box').slideDown();
	...
	$('#add-new-field-btn').on('click', function (e) {
		e.preventDefault();
		var field = $('#add-new-field-select').val();
		if (field == '-1') return;
		var field_data;
		...
		if (field_data != '') {
			var html = getFieldFormRow(field_data);
			$('#wciz-fields-form-section').append(html);
			disableSelectedOption(field, '#add-new-field-select');
			$('#add-new-field-select').val('-1');
			createPrimaryFieldForm();
			makeselectboxsearchable();
		}
	})

	function createPrimaryFieldForm(load = false) {

		var selected = $('#primary-field-select').val();
		if (load) {
			selected = $('#primary-field-select').attr('primary_field');
		}

		if (!selected && default_primary != '') {
			selected = default_primary;
		}

		var options = '<option value="">--Select--</option>';
		$.each($('.crm-field-name'), function (index, value) {
			var v = $(this).val();
			var label = $(this).closest('div').find('.field-label-txt').html();;
			s = v == selected ? 'selected' : '';
			options += '<option value="' + v + '" ' + s + '>' + label + '</option>';
		});
		$('#primary-field-select').html(options);
		$('#wciz-primary-field-section-wrapper').slideDown();
	}

	function disableSelectedOption(value, selectId) {
		var allOptions = $(selectId).find('option');
	...
	function maybeSetFieldMapping() {

		if ($('#wciz-feeds-zoho-object').length == 0) {
			return;
		}
		$('#wciz-feed-event-section-wrapper').slideDown();
		var module = $('#wciz-feeds-zoho-object').val();
		if (module == '-1') return;
		crm_selected_object = module;
		var event = 'get_module_fields';
		var request = { action, event, nonce, module };
		jQuery.post(ajax_url, request).done(function (response) {
			response = JSON.parse(response);
			if (response.fields) {
				crm_fields = response.fields;
				createFieldsForm(response.fields, module);
				createAddNewFieldForm(response.fields, true);
				createStatusMappingForm();
				createProductDetailForm();
	...
	function makeselectboxsearchable() {
		$('.field-value-select').select2();
		$('.custom-value-select').select2();
	}

	function createUserTypeForm() {
		$('#wciz-contact-user-type-section-wrapper').slideUp();


		if ('Contacts' != crm_selected_object) {
			return;
		}

		var selected_event = $('#select-feed-event').val();

		if ('user_update_create' == selected_event || 'send_manually' == selected_event) {
			return;
		}

		$('#wciz-contact-user-type-section-wrapper').slideDown();
	}

	function createTaxRateForm() {

		$('#wciz-tax-rate-section-wrapper').slideUp();

		if ('Products' != crm_selected_object) {
			return;
		}

		var showTaxSection = false;
		var field_data = {};
		if (crm_fields.length > 0) {
			for (let index = 0; index < crm_fields.length; index++) {
				const element = crm_fields[index];
				if (element.api_name == 'Tax') {
					showTaxSection = true;
					field_data = element;
					break;
				}
		...
		if (field_data.hasOwnProperty('pick_list_values')) {

			var tax_rate_select = $('.tax-rate-select');

			tax_rate_select.each(function () {
				var selected_rates = $(this).attr('selected_rates');
				var html = '<option value="">Select Zoho Tax Rate</option>';
				$.each(field_data.pick_list_values, function (index, value) {
					var s = '';
					if (selected_rates == value.actual_value) {
						s = 'selected';
					}
					html += '<option value="' + value.actual_value + '" ' + s + ' >' + value.display_value + '</option>';
				});
				$(this).html(html);
			});
		}
		...
		if (showTaxSection) {
			$('#wciz-tax-rate-section-wrapper').slideDown();
		}
	}

	function createProductImageForm() {

		$('#wciz-product-image-section-wrapper').slideUp();
		...
		$('#wciz-product-image-section-wrapper').slideDown();

	}

	function createSyncVariableProductForm() {

		$('#wciz-variable-product-section-wrapper').slideUp();
		...
		$('#wciz-variable-product-section-wrapper').slideDown();

	}

	function createFeedEventForm() {
		// if ( 'send_manually' != default_event ) {
		if (default_event != '') {
			$('#select-feed-event').val(default_event);
		}
		// }
		$('#wciz-feed-event-section-wrapper').slideDown();
	}

	function createProductDetailForm() {
		$('#wciz-product-detail-section-wrapper').slideUp();
		var showProductSection = false;
		var field_data = {};
		if (crm_fields.length > 0) {
			for (let index = 0; index < crm_fields.length; index++) {
				const element = crm_fields[index];
				if (element.api_name == 'Product_Details') {
		...
		if (showProductSection) {
			if (field_data.system_mandatory) {
				var cb = $('.add-line-item-cb');
				cb.prop('checked', true);
				cb.attr('system_mandatory', field_data.system_mandatory);
			}
			$('#wciz-product-detail-section-wrapper').slideDown();
		} else {
			var cb = $('.add-line-item-cb');
			if (cb.prop('checked')) {
	...
	$(document).on('click', '.contact-only-guest-feed', function (e) {
		var cb = $(this);
		if (cb.prop("checked") == true) {
			$('.wciz-form-wrapper-user-type').slideDown();
		} else {
			$('.wciz-form-wrapper-user-type').slideUp();
		}
	})


	$(document).on('click', '.add-line-item-cb', function (e) {
		if ($(this).attr('system_mandatory')) {
			e.preventDefault();
		}
	})

	function createStatusMappingForm() {

		var cb = $('.use-status-mapping-cb');
		if (cb.length == 0) return;

		if (cb.prop("checked") == true) {

			var status_mapping = $('#wciz-order-status-mapping-section-wrapper').attr('status_mapping');

			status_mapping = JSON.parse(status_mapping);
			if ($.isEmptyObject(status_mapping)) return;
			var field = cb.attr('field_name');
			var field_data = '';
			for (let index = 0; index < crm_fields.length; index++) {
				if (crm_fields[index].api_name == field) {
					field_data = crm_fields[index];
			...
			if (field_data != '') {
				$.each(field_data.pick_list_values, function (index, value) {
					var probability = value.hasOwnProperty('probability') ? value.probability : '';
					var display_value = value.display_value;
					if (probability !== '') {
						display_value = value.display_value + ' ( Probability : ' + probability + ' )';
					}
					options += '<option value="' + value.display_value + '">' + display_value + '</option>';
				});
				$('.crm-status-mapping-select').html(options);
			}

			var wrapper_td = $('#woo-status-table');
			var status_input_list = wrapper_td.find('.woo-status');
			$.each(status_input_list, function () {
				var input = $(this);
				input.closest('tr').find('.crm-status-mapping-select').val(status_mapping[input.val()]);
			});
			$('#wciz-order-status-mapping-section-wrapper').slideDown();
		}
	}

	function createAddNewFieldForm(fields, map_data = false) {
		$('#wciz-add-new-field-section-wrapper').slideUp();
		var mapping = {};
		if (map_data) {
			mapping = $('#wciz-fields-form-section').attr('mapping_data');
			mapping = JSON.parse(mapping);
		}

		var html = '<option value="-1">--Select Field--</option>';
		$.each(fields, function (index, value) {
			var disable = '';
			if (value.system_mandatory || mapping.hasOwnProperty(value.api_name) || value.api_name == 'Product_Details' || default_mapping.hasOwnProperty(value.api_name)) {
				disable = 'disabled';
			}
			html += '<option value="' + value.api_name + '" ' + disable + ' >' + value.field_label + '</option>';
		});
		$('#add-new-field-select').html(html);
		$('#wciz-add-new-field-section-wrapper').slideDown();
	}

	function createFieldsForm(fields, selected_object) {

		$('#wciz-fields-form-section-wrapper').slideUp()
		var html = '';
		var mapping = {};
		var crm_object = $('#wciz-fields-form-section').attr('crm_object');
		if (crm_object == selected_object) {
			mapping = $('#wciz-fields-form-section').attr('mapping_data');
			mapping = JSON.parse(mapping);
		}

		$.each(fields, function (index, value) {
			if ((value.system_mandatory || mapping.hasOwnProperty(value.api_name) || default_mapping.hasOwnProperty(value.api_name)) && value.api_name != 'Product_Details') {
				var mapping_data = mapping.hasOwnProperty(value.api_name) ? mapping[value.api_name] : {};
				var default_data = default_mapping.hasOwnProperty(value.api_name) ? default_mapping[value.api_name] : {};
				html += getFieldFormRow(value, mapping_data, default_data);
			}
		});
		$('#wciz-fields-form-section').html(html);
		$('#wciz-fields-form-section-wrapper').slideDown();

	}

	$(document).on('change', '#select-feed-event', function (e) {
		var feed_event = $('#select-feed-event').val();
		var event = 'changed_field_values_according_to_feed_event';
		var request = { action, event, nonce, feed_event };
		$.post(ajax_url, request).done(function (response) {
			response = JSON.parse(response);
	...
	function getFieldFormRow(field, mapping_data = {}, default_data = {}) {

		picklist_html = '';

		if (field.data_type == 'picklist' || field.data_type == 'multiselectpicklist') {
			$.each(field.pick_list_values, function (index, value) {
				var limiter = '';
				if (index < field.pick_list_values.length - 1) {
					limiter = ', ';
				}
				picklist_html += value.display_value + limiter;
			});
		}
		var html = '<div class="wciz-feeds__form-wrap wciz-fields-form-row">';
		html += '<div class="wciz-form-wrapper">';
		html += '<div class="wciz-fields-form-section-head">';
		...
		if (!field.system_mandatory) {
			html += '<span class="field-delete dashicons"><img src="' + delete_icon + '"></span>';
		}
		html += '</div>';
		html += '<div class="wciz-fields-form-section-meta">';
		...
		html += '</div>';
		html += '<div class="wciz-fields-form-section-form">';
		if (!$.isEmptyObject(mapping_data)) {
			var hide_standard = (mapping_data.field_type == 'standard_field') ? '' : 'row-hide';
			var hide_custom = (mapping_data.field_type == 'custom_value') ? '' : 'row-hide';
			html += '<div class="wciz-fields-form-section-form">';
			...
			$.each(feed_settings.field_type.options, function (index, value) {
				var s = (mapping_data.field_type == index) ? 'selected' : '';
			...
			// 		var s = (mapping_data.field_value == index) ? 'selected' : '';  
			// 		html += '<option value="'+index+'" '+s+'>'+value+'</option>';
			// 	});

			// }
			// else{
			// 	$.each(feed_settings.field_value.options, function (index, value) { 
			// 		html += '<optgroup label="'+index+'">';
			// 		$.each(value, function (i, v) { 
			// 			var key = index+'_'+ i ; 
			// 			var s = (mapping_data.field_value == key) ? 'selected' : '';  
			...
			$.each(feed_settings.field_value.options, function (index, value) {
				...
				$.each(value, function (i, v) {
					var key = index + '_' + i;
					var s = (mapping_data.custom_field == key) ? 'selected' : '';
					html += '<option value="' + key + '" ' + s + ' >' + v + '</option>';
				});
				html += '</optgroup>';
			});

			// $.each(feed_settings.field_value.options.wp, function (index, value) { 
			//   var s = (mapping_data.custom_field == value) ? 'selected' : '';  
			//   html += '<option value="'+value+'" '+s+' >'+value+'</option>';
			// });

			html += '</select></div>';
			$.each(show_mapping, function (index, value) {
				if (value.crm_object == crm_selected_object && value.crm_field == field.api_name) {
					var c = (mapping_data.use_status_mapping == 'yes') ? 'checked' : '';
					html += '<div class="form-field-row"><label>Map with woo order statuses</label><input ' + c + ' type="checkbox" field_name="' + field.api_name + '" class="use-status-mapping-cb" value="yes" name="use_status_mapping[' + field.api_name + ']"></div>';
				}
			});
		}
		else if (!$.isEmptyObject(default_data)) {
			var hide_standard = (default_data.field_type == 'standard_field') ? '' : 'row-hide';
			var hide_custom = (default_data.field_type == 'custom_value') ? '' : 'row-hide';
			html += '<div class="wciz-fields-form-section-form">';
			html += '<div class="form-field-row row-field-type">';
			html += '<label>' + feed_settings.field_type.label + '</label>';
			html += '<select class="field-type-select" name="field_type[]">';
			$.each(feed_settings.field_type.options, function (index, value) {
				var s = (default_data.field_type == index) ? 'selected' : '';
				html += '<option value="' + index + '" ' + s + '>' + value + '</option>';
			});
			html += '</select></div>';
			html += '<div class="form-field-row row-field-value row-standard_field ' + hide_standard + '">';
			...
			// 	$.each(boolean, function (index, value) { 
			// 		var s = (default_data.field_value == index) ? 'selected' : '';
			// 		html += '<option value="'+index+'" '+s+'>'+value+'</option>';
			// 	});
			...
			// 		$.each(value, function (i, v) { 
			// 			var key = index+'_'+ i ; 
			// 			var s = (default_data.field_value == key) ? 'selected' : '';  
			// 			html += '<option value="'+key+'" '+s+' >'+v+'</option>';
			// 		});
			// 		html += '</optgroup>';
			// 	});
			...
			//     //  $.each(feed_settings.field_value.options.wp, function (index, value) { 
			//     //    var s = (default_data.field_value == value) ? 'selected' : '';  
			//     //    html += '<option value="'+value+'" '+s+' >'+value+'</option>';
			//     //  });
			// }
			$.each(feed_settings.field_value.options, function (index, value) {
			...
			html += '<div class="form-field-row row-custom_value row-field-value ' + hide_custom + ' ">';
			html += '<label>' + feed_settings.custom_value.label + '</label>';
			html += '<input type="text" class="custom-value-input" name="custom_value[]" value="' + default_data.custom_value + '" />';
			html += '<select class="custom-value-select" name="custom_field[]" >';
			...
			$.each(feed_settings.field_value.options, function (index, value) {
				...
				$.each(value, function (i, v) {
					var key = index + '_' + i;
					var s = (default_data.custom_field == key) ? 'selected' : '';
					html += '<option value="' + key + '" ' + s + ' >' + v + '</option>';
				});
				html += '</optgroup>';
			});
			...
			//  $.each(feed_settings.field_value.options.wp, function (index, value) { 
			//    var s = (default_data.custom_field == value) ? 'selected' : '';  
			//    html += '<option value="'+value+'" '+s+' >'+value+'</option>';
			//  });
			html += '</select></div>';
			$.each(show_mapping, function (index, value) {
				if (value.crm_object == crm_selected_object && value.crm_field == field.api_name) {
					var c = (default_data.use_status_mapping == 'yes') ? 'checked' : '';
					html += '<div><label>Map with woo order statuses</label><input ' + c + ' type="checkbox" field_name="' + field.api_name + '" class="use-status-mapping-cb" value="yes" name="use_status_mapping[' + field.api_name + ']"></div>';
				}
			});
			...
			$.each(show_mapping, function (index, value) {
				if (value.crm_object == crm_selected_object && value.crm_field == field.api_name) {
					html += '<div><label>Map with woo order statuses</label><input type="checkbox" field_name="' + field.api_name + '" class="use-status-mapping-cb" value="yes" name="use_status_mapping[' + field.api_name + ']"></div>';
				}
			});
		}
		if (field.data_type == 'lookup') {
			html += '<div ><p class="wciz-desc" >Please map this field with an existing feed and make sure to trigger that feed before this feed.</p></div>';
		}
		html += '</div></div></div></div>';
		return html;
	}

	function makeRemovedFieldAvlb(field_name) {
		var allOptions = $('#add-new-field-select').find('option');
		$.each(allOptions, function () {
	...
	$(document).on('click', '.wciz-fields-form-row .field-delete', function (e) {
		var form_row = $(this).closest('.wciz-fields-form-row');
		var field_name = form_row.find('.crm-field-name').val();
		form_row.slideUp('400', function () {
			makeRemovedFieldAvlb(field_name);
			form_row.remove();
			createPrimaryFieldForm();
		});
	})

	$(document).on('click', '.use-status-mapping-cb', function (e) {
		if ($(this).prop("checked") == true) {
			...
			var event = 'get_default_status_mapping_data';
			var request = { event, crm_selected_object, field, action, nonce }
			var default_status = {};
			...
			$.each(status_input_list, function () {
				var input = $(this);
				var selected = default_status.hasOwnProperty(input) ? default_status[input] : false;
				input.closest('tr').find('.crm-status-mapping-select').html(options);
				if (selected !== false) {
					input.closest('tr').find('.crm-status-mapping-select').val(selected);
				}
			});
			...
			$('#wciz-order-status-mapping-section-wrapper').slideUp();;
		}
	})

	$('#wciz-woo-zoho-refresh-object').on('click', function (e) {
		var button = $(this);
		var event = 'get_modules';
		var force = true;
		var request = { action, event, nonce, force };
		button.addClass('wciz-btn__loader');
		$.post(ajax_url, request).done(function (response) {
			response = JSON.parse(response);
			if (response.hasOwnProperty('modules')) {
				var options_html = createModuleSelect(response.modules);
				$('#wciz-feeds-zoho-object').html(options_html);
				button.removeClass('wciz-btn__loader');
			}
		})
	})

	$(document).on('click', '#wciz-save-feed-btn', function (e) {
		e.preventDefault();
		var feed_data = {};
		var title = $('#feed-title').val();
		var crm_object = $('#wciz-feeds-zoho-object').val();
		if (title == '') {
			triggerError('Please enter title for feed');
			return
		}
		if (crm_object == '-1') {
			triggerError('Please select a object');
			return
		}
		var mapping = [];
		var wrapper = $('.wciz-fields-form-row');
		$.each(wrapper, function (index, value) {
			var row_mapping = [];
			var row = $(this);
			var input_ele = row.find('input, select');
			var field = {};
			$.each(input_ele, function (i, v) {
				var ele = $(this);
				var row_wrapper = $(this).closest('.form-field-row');
				var name = ele.attr('name');
				var value = ele.val();
				if (!(row_wrapper.hasClass('row-hide')) && typeof (name) != undefined) {
					field[name] = value;
				}
			})
			row_mapping.push(field)
			mapping.push(row_mapping)
		});
		var meta = '';
		var settings = '';
		feed_data = { title, crm_object, mapping, meta, settings };
		var event = 'save_feed_data';
		$.post(ajax_url, { feed_data, event, action, nonce }).done(function (response) {

		})

	})

	$('#wciz-woo-zoho-refresh-fields').on('click', function (e) {
		var module = $('#wciz-feeds-zoho-object').val();
		if (module == '-1') {
			triggerError('Please select an object');
			return;
		}
		var button = $(this);
		...
		var event = 'get_module_fields';
		var force = true;
		var request = { action, event, nonce, module, force };
		button.addClass('wciz-btn__loader')
		jQuery.post(ajax_url, request).done(function (response) {
			response = JSON.parse(response);
			if (response.fields) {
				crm_fields = response.fields;
				createFieldsForm(response.fields, module);
				createAddNewFieldForm(response.fields);
				makeselectboxsearchable();
				button.removeClass('wciz-btn__loader')
			}
		})
	...
	$(document).on('change', '.field-type-select', function (e) {
		...
		var field_type = type_select.val();
		var wrapper = type_select.closest('.wciz-fields-form-row');
		wrapper.find('.row-field-value').addClass('row-hide');
		wrapper.find('.row-' + field_type).removeClass('row-hide');
	})

	$(document).on('change', '.custom-value-select', function (e) {
		var type_select = $(this);
		var field_value = type_select.val();
		var wrapper = type_select.closest('.wciz-fields-form-row');
		var input = wrapper.find('.custom-value-input').val();
		wrapper.find('.custom-value-input').val(input + '{' + field_value + '}');
	})

	$('.post-type-wciz_crm_feed #publish').on('click', function (e) {
		var title = $('#title').val();
		var crm_object = $('#wciz-feeds-zoho-object').val();
		if (title == '') {
			triggerError('Please enter title for feed');
	...
	$('.create-default-feeds').on('click', function (e) {
		e.preventDefault();
		create_feed(step);
	})

	function create_feed(step = 0) {
		var event = 'create_default_feeds';
		$.post(ajax_url, { action, event, step, nonce }).done(function (response) {
			response = JSON.parse(response);
			if (response.success) {
				if (response.next_step < response.total_count) {
					create_feed(response.next_step);
				}
				onboardingFeeds.push({ 'id': response.feed_id, 'type': response.feed_type });
				var perWidth = (response.next_step / response.total_count) * 100;
				moveProperties(perWidth, response.message);
			}
		});
	}

	function createModuleSelect(modules) {
		var html = '<option value="-1">--Select Object--</option>';
	...
	$('.wciz-crm-feed-chk').on('click', function (e) {
		var feed_id = $(this).attr('feed_id');
		if (feed_id) {
			var status = $(this).prop('checked') ? 'publish' : 'draft';
			var event = 'change_feed_status';
			var request = { action, event, nonce, feed_id, status };
			$.post(ajax_url, request).done(function (response) {
	...
	$('.wciz-delete-feed-btn').on('click', function (e) {
		e.preventDefault();
		var btn = $(this);
		var feed_id = btn.attr('feed_id');
		var list_item = btn.closest('.wciz-about__list-item');
		var event = 'delete_feed_post';
		$.post(ajax_url, { action, event, nonce, feed_id }).done(function (response) {
			list_item.slideUp();
		})
	})

	$('#wciz-woo-zoho-refresh-access-token').on('click', function (e) {
		e.preventDefault();
		var event = 'renew_access_token';
		$.post(ajax_url, { action, event, nonce }).done(function (response) {
			response = JSON.parse(response)
			if (response.success) {
				triggerSuccess(response.msg);
				$('#wciz-zoho-token-notice').html(response.token_message);
			} else {
	...
	$('#wciz-woo-zoho-disconnect').on('click', function (e) {
		e.preventDefault();

		swal(disconnect.title, {
			buttons: {
				disconnect: disconnect.yes,
				cancel: disconnect.no,
			},
		}).then((value) => {
			switch (value) {

				case "disconnect":
					swal(disconnect.processing);
					var event = 'disconnect_account';
					$.post(ajax_url, { action, event, nonce }).done(function (response) {
						location.reload();
					});
					break;
	...
	$('.wciz-refresh-order-status').on('click', function (e) {
		var button = $(this);
		var event = 'get_zoho_salesorder_status';
		var force = true;
		var request = { action, event, nonce, force };
		button.addClass('wciz-btn__loader')
		jQuery.post(ajax_url, request).done(function (response) {
			...
			jQuery.each(response, function (key, values) {
				html += '<tr> <th scope="row" class="titledesc">' + values.display_value + '</th><td>';
				html += '<select class="wciz-zoho-select" name="wciz_zoho_zoho_status[]" zoho_status="' + values.actual_value + '">';
				$.each(order_status, function (index, value) {
					var selected = (mapped_zoho_status[values.actual_value] == index) ? 'selected' : '';
					html += '<option value="' + index + '" ' + selected + '>' + value + '</option>';
				});
				html += '</select></td></tr>';
			});
			$('.wciz-zoho-status-mapping-table tbody').html(html);
			button.removeClass('wciz-btn__loader');
		}).fail(function (response) {
			triggerError(ajax_data.ajax_error);
			button.removeClass('wciz-btn__loader');
		});
	});

	$('#wciz-woo-zoho-reauth').on('click', function (e) {
		e.preventDefault();
		var event = 'get_reauthorize_url';
		$.post(ajax_url, { action, event, nonce }).done(function (response) {
			response = JSON.parse(response)
			if (response.success) {
	...
	$(document).on('click', '.progress-next', function () {
		var cl = $(".wciz-progress__bar").attr("class").split(/\s/).filter(function (cn) {
			return cn.indexOf('step') === 0;
		});
		var step = parseInt(cl[0].split('-')[1]) + 1;
		var newclass = "step-" + step;
		$(".wciz-progress__bar").removeClass(cl[0]).addClass(newclass);
	});

	$('.show-properties-btn').on('click', function (event) {
		event.preventDefault();
		jQuery('.wciz-intro').slideUp('400', function () {
			jQuery('.wciz-properties').slideDown();
		});
	})

	$('.show-intro-progress').on('click', function (e) {
		e.preventDefault();
		showIntroProgress(e)
		create_feed();
		//moveProperties();
	...
	$('.wciz_next_step').on('click', function (e) {
		e.preventDefault();
		jQuery('.wciz-properties').slideUp('400', function () {
			jQuery('.wciz-abandoned-cart').slideDown();
		});
	})

	$('.create-abandoned-cart-object-and-fields').on('click', function (e) {
		e.preventDefault();
		jQuery('.wciz-abandoned-cart').slideUp('400', function () {
			jQuery('.wciz-product-initial-sync').slideDown();
		});
	})

	$('#skip_abandoned_cart_creation').on('click', function (e) {
		e.preventDefault();
		jQuery('.wciz-abandoned-cart').slideUp('400', function () {
			jQuery('.wciz-product-initial-sync').slideDown();
		});
	...
	$('.sync-products-initially').on('click', function (e) {
		e.preventDefault();
		jQuery('.wciz-product-initial-sync').slideUp('400', function () {
			jQuery('.wciz-sync').slideDown();
		});
	})

	$('#skip_product_initial_syncing').on('click', function (e) {
		e.preventDefault();
		jQuery('.wciz-product-initial-sync').slideUp('400', function () {
			jQuery('.wciz-sync').slideDown();
		});
	...
	function showIntroProgress(event) {
		jQuery('.wciz-properties__progress').addClass('active');
		jQuery('.wciz-properties__progress-button').hide();
	};

	function moveProperties(width = 0, message) {
		var elem = document.getElementById("wciz-properties__progress-bar");
		elem.style.width = width + '%';
		elem.innerHTML = width * 1 + '%';
		jQuery('.wciz-properties__progress-result').html(message);
		if (100 == width) {
			jQuery('.wciz-properties__button').addClass('active');
		}
	};

	// jQuery('.wciz-dashboard__header-arrow').on('click', function (e) {
	// 	jQuery('.wciz-dashboard__header-content').toggleClass('active');
	// 	jQuery('.wciz-dashboard__header-button-wrap').slideToggle('medium', function () {
	// 		if (jQuery(this).is(':visible'))
	// 			jQuery(this).css('display', 'flex');
	// 	});
	// });

	jQuery('#wciz-zoho-domain').on('change', function (e) {
		var domain = jQuery(this).val();
		console.log(domain);
		if ('ca' == domain) {
			jQuery('#wciz-zoho-use-global-app').prop('checked', false);
			handle_auth_field();
		}
	});

	jQuery('#wciz-zoho-authorize-button').on('click', function (e) {
		e.preventDefault();
		href = jQuery('#wciz-zoho-authorize-button').attr('href');
		client_id = jQuery('#wciz-zoho-client-id').val();
		secret_id = jQuery('#wciz-zoho-secret-id').val();
		domain = jQuery('#wciz-zoho-domain').val();
		var use_gapp = jQuery('#wciz-zoho-use-global-app').prop('checked') ? true : false;

		// Incase using custom app.
		if (true == use_gapp) {
			if (domain.length == 0) {
				return false;
			}
				...
				href = updateQueryStringParameter(href, 'use_gapp', use_gapp);
				jQuery('#wciz-zoho-authorize-button').attr('href', href);
	...
	/**
	 * Bulk DataSync Scripts Start
	 * 
	 */

	// Current form submissions data.
	let currentFormObj, currentForm, currentEvent, isOnboarding = false, synced_total = 0;

	// Show Sync Type select form.
	$(document).on('click', '.wciz-sync-start', function (e) {

		let initiateAction = jQuery(this).attr('data-form');
		let selectedForm = '.form_' + initiateAction.trim();
		if (!jQuery(this).hasClass('active')) {
			if (jQuery(this).hasClass('is_hidden')) {
				jQuery(this).children('img').css('transform', 'rotate(0deg)');
				jQuery(selectedForm).parent('.wciz-about__list-item-text').addClass('is_hidden');
				jQuery(selectedForm).find('.wciz-sync-form__field:first-child').addClass('is_hidden');
				jQuery(this).removeClass('is_hidden');
			} else {
				jQuery(this).addClass('is_hidden');
				jQuery(this).children('img').css('transform', 'rotate(180deg)');
				jQuery('.wciz-lists__progress').hide();
				jQuery(selectedForm).parent('.wciz-about__list-item-text').removeClass('is_hidden');
				jQuery(selectedForm).find('.wciz-sync-form__field:first-child').removeClass('is_hidden');
			}
		}

	});

	$(document).on('click', '.wciz-about__list-item-text-1', function (e) {

		let initiateAction = jQuery(this).attr('data-form');
		let selectedForm = '.form_' + initiateAction.trim();
		if (!jQuery(this).hasClass('active')) {
			if (jQuery(this).hasClass('content_display')) {
				jQuery(this).next().children('a').children('img').css('transform', 'rotate(0deg)');
				jQuery(selectedForm).parent('.wciz-about__list-item-text').addClass('is_hidden');
				jQuery(selectedForm).find('.wciz-sync-form__field:first-child').addClass('is_hidden');
				jQuery(this).removeClass('content_display');
			} else {
				jQuery(this).addClass('content_display');
				jQuery(this).next().children('a').children('img').css('transform', 'rotate(180deg)');
				jQuery('.wciz-lists__progress').hide();
				jQuery(selectedForm).parent('.wciz-about__list-item-text').removeClass('is_hidden');
				jQuery(selectedForm).find('.wciz-sync-form__field:first-child').removeClass('is_hidden');
			}
	...
	$(document).on('change', '#wciz-crm-select-woo-object', function (e) {
		if ('Products' == $(this).val() && (false == $('#wciz-crm-select-woo-object-date-to').parent().hasClass('is_hidden'))) {
			$('#wciz-crm-select-woo-object-date-to').parent().addClass('is_hidden');
			$('#wciz-crm-select-woo-object-date-from').parent().addClass('is_hidden');
		}
	});

	// Selection for data sync.
	$(document).on('change', '.wciz-crm-select-sync-fields', function (e) {

		let selectedForm = jQuery(this).closest('form');

		// Bulk or One click.
		let syncType = selectedForm.attr('class').split(' ').pop();
		if ('form_bulk_data_sync' == syncType) {
			if ('wciz-crm-select-object-feed' == $(this).attr('id') && 'wciz-crm-select-woo-object-date-from' == $(this).parent().next().children('input').attr('id') && 'Products' != $(this).parent().prev().children('select').val()) {
				$(this).parent().append('<p style="margin-top:50px;"><b><strong>NOTE: </strong></b>' + date_range_note + '</p>');
				$(this).parent().nextAll('.wciz-sync-form__field').wrapAll('<div class="from_to_date"></div>');
			}

			if ('Products' == $(this).parent().prev().children('select').val()) {
				if ($(this).parent().has('p')) {
					$(this).parent().children('p').remove();
				}
				jQuery(selectedForm).find('.from_to_date').children().hide();
			} else if ('wciz-crm-select-object-feed' == $(this).attr('id')) {
				if (0 == $(this).parent().has('p').length) {
					$(this).parent().append('<p style="margin-top:50px;"><b><strong>NOTE: </strong></b>' + date_range_note + '</p>');
				}
				jQuery(selectedForm).find('.from_to_date').children().show();
			}
		}


		//    wciz-crm-select-woo-object-date-from

		// Whether to select feed or to show sync Button.
		let syncStep = jQuery(this).attr('name');
		// Changed Event is to get Feed Ids.
		if ('wciz-crm-select-woo-object' == syncStep) {

			let syncModule = jQuery(this).val();
			let event = 'get_object_feed_options';
			var request = { action, event, nonce, syncModule };

			jQuery.post(ajax_url, request).done(function (response) {

				if (200 == response.result) {
					let feedSelectWrap = jQuery(selectedForm).find('.wciz-sync-form__field:nth-child(2)');
					let feedSelect = feedSelectWrap.find('select');
					feeds = response.options;

					// Add feeds.
					let html = '<option value="" selected="selected">Choose option</option>';
					feedSelect.html(html);
					...
					feedSelectWrap.removeClass('is_hidden');
					$('.wciz-start-sync-action').show();
				} else {
					$('.wciz-start-sync-action').hide();
					triggerError("No Active Feeds Found for " + syncModule.replace('_', ' ') + "!");
				}
			});
		} else if (('wciz-crm-select-object-feed' == syncStep) && ('form_bulk_data_sync' == syncType) && ('Products' != $('#wciz-crm-select-woo-object').val())) {
			let feedSelectWrap = jQuery(selectedForm).find('.from_to_date').children();
			// let feedSelectWrap2 =  jQuery(selectedForm).find('.from_to_date');

			feedSelectWrap.removeClass('is_hidden');
			// feedSelectWrap2.removeClass('is_hidden');
			selectedForm.find('.wciz-start-sync-action').removeClass('is_hidden');

		} else {  // Changed Event is to start Feed Sync button.

			// Show Start Button and Submit the form.
			selectedForm.find('.wciz-start-sync-action').removeClass('is_hidden');
		}
	});

	// Submit current form.
	// $(document).on('click', '.wciz-start-sync-action', function(e){
	//   e.preventDefault();
	//   jQuery(this).closest('form').trigger('submit');
	// });

	// Submission for bulk/one click handling.
	$(document).on('submit', '.wciz-sync-process-form', function (e) {

		e.preventDefault();

		// Bulk or One click.
		let syncType = jQuery(this).attr('class').split(' ').pop();

		currentFormObj = jQuery(this);
		currentForm = currentFormObj.serialize().replaceAll('wciz-crm-select-', '');
		//currentEvent = getEventCallback(syncType);
		currentEvent = syncType;
		isDataSync = 'true';

		synced_total = 0;
		resultTab = currentFormObj.next('.wciz-sync-result');

		// Reset values.
		resultTab.find('span').html('');

		// Initialise batch one.
		CaptureAllBatchRequired();
	});

	// Initiate sync ajax.
	function CaptureAllBatchRequired() {

		// If bulk sync initiated on onboarding.
		if (true == isOnboarding) {
			// Just loop the crm objects.
			currentForm = 'woo-object=' + onboardingFeeds[currentSyncIndex].type + '&object-feed=' + onboardingFeeds[currentSyncIndex].id;
			currentEvent = "form_bulk_data_sync";
		}
	...
	// Next offset callback.
	function processNextBatch(response) {
		response = JSON.parse(response);
		result = response.result;
		message = response.message;

		if (true == result) {
			total_batch = message.total_batch;
			nextOffset = message.offset;

			if (nextOffset <= total_batch) {

				try {
					// Get next batch
					doTheCapture(nextOffset);
				} catch (error) {
	...
	// Define function for all calls.
	function doTheCapture(offset = 1) {
		...
		$.ajax({
			...
			function () {
				processNextBatch(processedResponse);
			}, function (error) {
	...
	function showResult(response) {
		...
		// Same Data all over request.
		total_posts = response.total_posts;
		total_batch = response.total_batch;
		batch_percent = 100 / total_batch;

		// If nothing found.
		if (0 == total_posts) {

			triggerSuccess('No Data Found');
			return;
		}

		// Dynamic Data after each request.
		nextOffset = response.offset;
		currentOffset = nextOffset - 1;

		percent = (currentOffset * batch_percent).toFixed(2);
		request = message.request;

		if ('100.00' == percent || '100' == percent) {
			synced_total = total_posts;
		} else {
			synced_total = synced_total + request.length;
		}
		if (true == isOnboarding) {


			if ('undefined' == typeof onboardingFeeds[currentSyncIndex]) {
				return;
			}

			// Add data in progress bar.
			moveLists(onboardingFeeds[currentSyncIndex].type, percent);

			// Show Updated Details.
			objectType = onboardingFeeds[currentSyncIndex].type;
			lastObject = objectType;

			let progressBarResult = jQuery("#wciz-lists__progress-bar-" + objectType + "-result");
			progressBarResult.html('<span class="wciz-lists__progress-result">(' + synced_total + ' out of ' + total_posts + ')</span>');

		} else {

			// Hide Form 
			currentFormObj.addClass('is_hidden');

			// Show progress bar
			resultBarWrap = currentFormObj.next('.wciz-lists__progress');
			resultBar = resultBarWrap.find('.wciz-lists__progress-bar');
			resultTab = resultBarWrap.next('.wciz-sync-result');

			resultBarWrap.show();
			resultBar.css('width', percent + '%');
			resultBar.html(percent + '%');

			if ('100.00' == percent || '100' == percent) {
				triggerSuccess('Objects synchronized. Verify logs for confirmation.');
			}

			// Show values.
			resultTab.html('<span class="wciz-lists__progress-result">(' + synced_total + ' out of ' + total_posts + ')</span>');
		}
	}

	// Reset Datasync forms.
	function resetDataSyncCenter() {

		// Hide All form fields wrapper.
		jQuery('.wciz-crm-select-sync-fields').each(function () {
			jQuery(this).parent().addClass('is_hidden');
		});

		jQuery('form > .wciz-about__list-item-text').each(function () {
			jQuery(this).addClass('is_hidden');
		});

		// Show all buttons
		jQuery('.wciz-sync-start').removeClass('is_hidden');
	}

	// Get Event callback name.
	function getEventCallback(string) {
		var capitalized = string.replace(/(^|[\s-])\S/g, function (match) {
			if (match.includes('-')) {
		...
		var event = capitalized.replaceAll('-', '');
	...
	jQuery('.wciz-onboarding-complete').on('click', function (e) {
		e.preventDefault();
		var event = 'mark_onboarding_complete'
		$.post(ajax_url, { action, nonce, event }).done(function (response) {
			location.reload();
		})
	...
	jQuery('.inside .wciz-content-wrap a').on('click', function (e) {
		e.preventDefault();
		jQuery(this).toggleClass('active');
		jQuery(this).next('.wciz-feeds__meta-box-main-wrapper').toggle(100);
	});

	$(".wciz-btn__loader").on('click', function (e) {
		$(this).toggleClass("active");
	})

	$('#wciz-zoho-setting-button').on('click', function (e) {
		e.preventDefault();
		var enable_plugin = $('#wciz-zoho-enable-plugin').prop('checked') ? 'yes' : 'no';
		var enable_sync = $('#wciz-zoho-enable-sync').prop('checked') ? 'yes' : 'no';
		var background_sync_allowed = $('#wciz-zoho-background-sync').prop('checked') ? 'yes' : 'no';
		var enable_log = $('#wciz-zoho-enable-log').prop('checked') ? 'yes' : 'no';
		var delete_log = $('#wciz-zoho-delete-log').val();
		var wipe_data = $('#wciz-zoho-wipe-data').prop('checked') ? 'yes' : 'no';
		var update_stock = $('#wciz-zoho-update-stock').prop('checked') ? 'yes' : 'no';
		var update_order_status = $('#wciz-zoho-update-order-status').prop('checked') ? 'yes' : 'no';
		var enable_data_log = $('#wciz-zoho-enable-data-log').prop('checked') ? 'yes' : 'no';
		var enable_reduce_stock = $('#wciz-zoho-enable-reduce-stock').prop('checked') ? 'yes' : 'no';
		var enable_discount_in_line_items = $('#wciz-zoho-enable-discount-in-line-items').prop('checked') ? 'yes' : 'no';
		var enable_discount_in_both = $('#wciz-zoho-enable-discount-in-line-items-parallel-subtotal').prop('checked') ? 'yes' : 'no';
		var enable_discount_in_subtotal = $('#wciz-zoho-enable-discount-in-subtotal').prop('checked') ? 'yes' : 'no';
		var enable_product_deletion_woo_to_zoho = $('#wciz-zoho-enable-product-deletion-woo-to-zoho').prop('checked') ? 'yes' : 'no';
		var enable_product_deletion_zoho_to_woo = $('#wciz-zoho-enable-product-deletion-zoho-to-woo').prop('checked') ? 'yes' : 'no';
		var enable_product_syncing_zoho_to_woo = $('#wciz-zoho-enable-product-syncing-zoho-to-woo').prop('checked') ? 'yes' : 'no';
		var enable_product_creation_zoho_to_woo = $('#wciz-zoho-enable-product-creation-zoho-to-woo').prop('checked') ? 'yes' : 'no';
		var status_mapping = {};

		if ('yes' == update_order_status) {
			$('select[name="wciz_zoho_zoho_status[]"]').each(function (index, member) {
				status_mapping[$(member).attr('zoho_status')] = $(member).val();
			});
		}

		if (delete_log < 7) {
			swal("Error", "Delete log after x days can't be less than 7", "error");
			return;
		}
		...
		var settings = {
			...
			enable_data_log,
			enable_reduce_stock,
			enable_discount_in_line_items,
			enable_discount_in_both,
			enable_discount_in_subtotal,
			enable_product_deletion_woo_to_zoho,
			enable_product_deletion_zoho_to_woo,
			enable_product_syncing_zoho_to_woo,
			enable_product_creation_zoho_to_woo,
			status_mapping
		};

		var event = 'save_general_setting';
		$.post(ajax_url, { action, event, nonce, settings }).done(function (response) {
			swal("Success", "Settings Saved", "success").then(function () {
				if ('#wciz-settings' == window.location.hash) {
					location.reload();
				}
			});
		})
	...
	$('#wciz-zoho-abandoned-cart-setting-button').on('click', function (e) {
		e.preventDefault();
		var enable_abandoned_cart_sync = $('#wciz-zoho-enable-abandoned-cart-sync').prop('checked') ? 'yes' : 'no';
		var cart_timer = $('#wciz-zoho-cart-timer').val();
		var delete_data = $('#wciz-zoho-delete-data').val();
		var save_html_encoded = $('#wciz-zoho-save-html-encoded').prop('checked') ? 'yes' : 'no';
		var abn_track_data = $("#wciz-zoho-abn-track-data").find("option:selected").val();
		var abn_data_fields = $('#wciz-zoho-abn-data-fields').prop('checked') ? 'yes' : 'no';
		...
		var event = 'save_abandoned_cart_setting';
		$.post(ajax_url, { action, event, nonce, settings }).done(function (response) {
			swal("Success", "Settings Saved", "success").then(function () {
				//    if ( '#wciz-abandoned-carts' == window.location.hash ) {
				location.reload();
				//    }
			});
		})
	...
	$('#wciz-zoho-update-order-status').on('click', function (e) {
		if ($(this).prop('checked')) {
			$('.wciz-zoho-status-mapping-wrapper').slideDown();
		} else {
			$('.wciz-zoho-status-mapping-wrapper').slideUp();
		}
	})

	$('#wciz-woo-zoho-clear-log').on('click', function (e) {
		e.preventDefault();
	...
	$('#wciz-woo-zoho-download-log').on('click', function (e) {
		e.preventDefault();
	...
	/* Download the log created for product quantity update and order status */
	$('#wciz-zoho-download-data-log').on('click', function (e) {
		e.preventDefault();
		var event = 'download_data_log';
		var button = $(this);
		button.addClass('wciz-btn__loader');
		$.post(ajax_url, { action, nonce, event }).done(function (response) {
			response = JSON.parse(response);
	...
	/* Clear the log created for product quantity update and order status */
	$('#wciz-zoho-clear-data-log').on('click', function (e) {
		e.preventDefault();
		var event = 'clear_data_log';
		var button = $(this);
		button.addClass('wciz-btn__loader');
		$.post(ajax_url, { action, nonce, event }).done(function (response) {
			response = JSON.parse(response);
	...
	$('#wciz-woo-zoho-reload-data-log').on('click', function (e) {
		// 	e.preventDefault();
		// console.log($('#wciz-zoho-data-log-content'));
		get_zoho_to_woo_sync_log();
	})

	// log page redirect for pagination
	...
	if (null !== paged) {
		if ('wciz-abandoned-carts' != result && 'wciz-dashboard' != result && 'wciz-feeds' != result && 'wciz-data-sync' != result && 'wciz-logs' != result && 'wciz-settings' != result && 'wciz-support' != result) {
			window.location.href = url + '#wciz-abandoned-carts';
			location.reload();
		}
	}
	// log page redirect for pagination
	...
	//Initial Synce
	jQuery('#wciz-start-initial-sync').on('click', function (e) {
		e.preventDefault();

		var prod_count = jQuery('.wciz-zoho-product-count').val();
		var order_count = jQuery('.wciz-zoho-shop-order-count').val();

		if (1000 < prod_count || 1000 < order_count) {
			...
			swal(skip.title, {
				...
				switch (value) {
					...
					default:
						...
						jQuery('#wciz-start-skip-setup').trigger('click');
						break;
				}
			});
	...
	function initOnboardingSync() {
		//jQuery('#wciz-start-skip-setup').hide();
		jQuery('.wciz-lists__button-start').hide();
		jQuery('.wciz-lists-Products-sync').slideDown('400', function () {
			startInitialSyncV2();
		});
	}

	function startInitialSync() {
		if (currentSyncIndex > 3) {
			jQuery('.wciz-lists__button .progress-next').addClass('active');
			return;
		}
		currentObject = onboardingFeeds[currentSyncIndex].type;
		currentId = onboardingFeeds[currentSyncIndex].id;
		jQuery('.wciz-lists-' + currentObject + '-sync').slideDown('400', function () {

			jQuery('html, body').animate({
				scrollTop: jQuery('.wciz-lists-' + currentObject + '-sync').offset().top
			}, 2000, function () {
				synced_total = 0;
				currentForm = 'woo-object=' + currentObject + '&object-feed=' + currentId;
				currentEvent = "form_bulk_data_sync";
				doTheInitCapture();
			});
	...
	function processInitNextBatch(response) {
		response = JSON.parse(response);
		result = response.result;
		message = response.message;
	...
	function doTheInitCapture(offset = 1) {
		...
		$.ajax({
			...
			success: function (response) {
				...
				message = response.message;
				if (true == result) {
					showInitResult(message);
				}
			}
		}).then(
			function () {
				processInitNextBatch(processedResponse);
			}, function (error) {
				console.error(error);
			}
		);
	...
	function showInitResult(response) {
	...
	function updateProgressBar() {
		var progressBar = jQuery("#wciz-lists__progress-bar-" + currentObject);
		progressBar.css('width', percent + '%');
		progressBar.html(percent * 1 + '%');
		var progressBarResult = jQuery("#wciz-lists__progress-bar-" + currentObject + "-result");
		if (percent == 0) {
			progressBarResult.html('<span class="wciz-lists__progress-result">(  No data found )</span>');
		} else {
			progressBarResult.html('<span class="wciz-lists__progress-result">(' + synced_total + ' out of ' + total_posts + ')</span>');
		}
	...
	$('#wciz-zoho-manual-sync').on('click', function (e) {
		e.preventDefault();
	...
	const triggerSuccess = (message) => {
		swal("Process Completed", message, "success").then(function () {
			if ('#wciz-data-sync' == window.location.hash) {
				location.reload();
			}
		});
	}
	...
	/**
	 * Load feed filter on page load.
	 * 
	 * @since      1.0.0
	 * @returns    void
	 */
	const initiliazeFeedFilter = () => {
		let post_id = jQuery('#post_ID').val();
		let post_type = jQuery('#post_type').val();
		let crm_selected_object = jQuery('#wciz-feeds-zoho-object').val();
		if ('-1' != crm_selected_object && '' != post_id && 'wciz_crm_feed' == post_type) {
			let event = 'create_feed_filters';
			jQuery.post(ajax_url, { action, event, crm_selected_object, nonce }).done(function (response) {
				response = JSON.parse(response);
		...
		if (jQuery('body').hasClass('post-type-wciz_crm_feed')) {
			if (jQuery('#wciz-zoho-enable-add-condition').is(':checked')) {
				jQuery('.wciz-initial-filter').find('.condition-value-field').attr('required', true);
			} else {
				jQuery('.wciz-initial-filter').find('.condition-value-field').removeAttr('required');
			}
		}
	}
	...
	const createOrFilteredHtml = (index = 1) => {
		...
		html += '<div class="wciz-form-filter-row">';
		...
		jQuery.each(objectFields, function (index, value) {
			html += '<optgroup label="' + index + '">';
		...
		if (1 != index) {
			html += '<img src="' + delete_icon + '" class="dashicons-trash" alt="Trash" style="max-width: 20px;">';

		}

		html += '<button data-next-and-index="2" data-or-index="' + index + '" class="wciz-btn wciz-btn--filled condition-and-btn">Add "AND" filter</button>';
		html += '</div></div>';
	...
	$(document).on('change', '#wciz-crm-select-woo-object-date-to', function (e) {
		var from_time = $('#wciz-crm-select-woo-object-date-from').val();
		var to_time = $('#wciz-crm-select-woo-object-date-to').val();
		const date = new Date;
		if (from_time && to_time) {
			console.log(Date.parse(from_time));
			console.log(Date.parse(to_time));
			if (Date.parse(from_time) > Date.parse(to_time)) {
				$(this).val('');
				alert('To date can not be less than from date.');
			}
		}
	});
	$(document).on('change', '#wciz-crm-select-woo-object-date-from', function (e) {
		var from_time = $('#wciz-crm-select-woo-object-date-from').val();
		var to_time = $('#wciz-crm-select-woo-object-date-to').val();
		const date = new Date;
		if (from_time && to_time) {
	...
	$(document).on('click', '.wciz-start-sync-action', function (e) {
		...
		var flag = true;
		var form = button.closest('.wciz-sync-process-form');
		var feed_id = form.find('#wciz-crm-select-object-feed').val();
		var sync_type = button.attr('sync_type');

		if (feed_id.length == 0 || sync_type.length == 0) {
			return;
		}

		button.addClass('wciz-btn__loader');

		var event = 'fetch_required_sync_count';
		var sync_count = 0;
		jQuery.post(ajax_url, { action, event, nonce, feed_id, sync_type, from_time, to_time }).done(function (response) {
			response = JSON.parse(response);
			if (response.success) {
				var out_of = response.data.length;
				console.log(out_of);
				form.closest('.wciz-about__list-item').find('.wciz-sync-start').addClass('active');
				syncBulkDataV2(feed_id, sync_type, 0, form, sync_count, from_time, to_time, out_of, button, '');
			} else {
				button.removeClass('wciz-btn__loader');
	...
	$(document).on('click', '#create-shipping-product', function (e) {
		e.preventDefault();
		var button = $(this);
		button.addClass('wciz-btn__loader');
		var event = 'create_shipping_product';
		jQuery.post(ajax_url, { action, event, nonce }).done(function (response) {
			response = JSON.parse(response);
			if (response.success) {
				var html = response.message + ' ( ' + response.shipping_product_id + ' )';
				button.addClass('hide');
				jQuery('.shipping-product-status-msg').html(html);
				jQuery('.shipping-product-status-msg').removeClass('hide');
			}
			button.removeClass('wciz-btn__loader');
		})
	});

	$(document).on('click', '.wciz_cart_table_view_cart', function (e) {
		e.preventDefault();
		var cart_email = $(this).data("email");
		var user_type = $(this).data("user_type");
		var event = 'wciz_cart_table_view_cart';
		jQuery.post(ajax_url, { action, event, nonce, cart_email, user_type }).done(function (response) {
			$('#wciz_cart_table_cart_details').html(response);
			$('.cart_details_close').on("click", function () {
				$('#wciz_cart_table_cart_details').html("");
			});
		})
	});

	$(document).on('click', '#sync_abandoned_carts', function (e) {
		e.preventDefault();
		var event = 'sync_abndn_carts';
		jQuery.post(ajax_url, { action, event, nonce }).done(function (response) {
			triggerSuccess('Abandoned cart Synced. Check logs for any verification.');
		})
	});

	$(document).on('click', '#create_abandoned_cart_object', function (e) {
		e.preventDefault();
		var event = 'create_abndn_cart_object';
		var button = $(this);
		button.addClass('wciz-btn__loader');
		jQuery.post(ajax_url, { action, event, nonce }).done(function (response) {

			$('#skip_abandoned_cart_creation').hide();
			// $('.wciz-abandoned-cart__progress').show();
			$('.create-abandoned-cart-object-and-fields').show();
			// $('.wciz-abandoned-cart__progress-result').html('Abandoned cart object and its fields created');
			// $('.wciz-abandoned-cart__progress-bar').html('100%');
			...
			var event = 'create_feed_for_abndn_cart_object';
			jQuery.post(ajax_url, { action, event, nonce }).done(function (response) {
				response = JSON.parse(response);
				setTimeout(function () {
					button.removeClass('wciz-btn__loader');
					$('#create_abandoned_cart_object').hide();
					$('.wciz-abandoned-cart-feed__progress').show();
					$('.wciz-abandoned-cart-feed__progress-result').html(response.data);
					$('.wciz-abandoned-cart-feed__progress-bar').html('100%');
				}, 1000);
			})
		})
	});
	...
	$(document).on('click', '#sync_products_initially', function (e) {
		e.preventDefault();
		swal(sync_product.title, {
			buttons: {
				instant: sync_product.instant,
				background: sync_product.background,
			},
		}).then((value) => {
			switch (value) {

				case "instant":
					jQuery('#skip_product_initial_syncing').hide();
					var button = jQuery(this);
					var from_time = $('#wciz-crm-select-woo-object-date-from').val();
					var to_time = $('#wciz-crm-select-woo-object-date-to').val();
					...
					var flag = true;
					var form = button.closest('.wciz-sync-process-form');
					var event = 'fetch_prod_feed_id';
					jQuery.post(ajax_url, { action, event, nonce }).done(function (response) {
						var p_feed_id = response;
						var feed_id = p_feed_id;
						var sync_type = 'bulk_data_sync';
						...
						jQuery.post(ajax_url, { action, event, nonce, feed_id, sync_type, from_time, to_time }).done(function (response) {
							...
							if (response.success) {
								var out_of = response.data.length;
								form.closest('.wciz-about__list-item').find('.wciz-sync-start').addClass('active');
								syncBulkDataV2(feed_id, sync_type, 0, form, sync_count, from_time, to_time, out_of, button, value);
							} else {
								button.removeClass('wciz-btn__loader');
								triggerError(response.message);
							}
	...
	function syncBulkDataV2(feed_id, sync_type, step = 0, form, sync_count, from_time, to_time, out_of, button, value) {
		const event = 'sync_bulk_data_request';
		console.log('step', step);
		jQuery.post(ajax_url, { action, event, nonce, step, feed_id, sync_type, from_time, to_time }).done(function (response) {
			response = JSON.parse(response);
			if (response.success) {
				sync_count = sync_count + response.synced_data.length;
				updateProgressBarV2(response.percent, form, sync_count, out_of, button, value);
				if (response.has_next_step) {
					syncBulkDataV2(feed_id, sync_type, response.next_step, form, sync_count, from_time, to_time, out_of, button, value);
				}
			}
		});
	}

	function updateProgressBarV2(percent, form, sync_count, out_of, button, value) {
		// var button = jQuery(this);
		form.addClass('is_hidden');
		if ('instant' == value) {
			resultBarWrap = $('.wciz-product-initial-sync__progress');
			resultBar = resultBarWrap.find('.wciz-product-initial-sync__progress-bar');
			resultTab = resultBarWrap.next('.wciz-product-initial-sync-result');
			resultBarWrap.show();
			resultBar.css('width', percent + '%');
			resultBar.html(percent + '%');

			if ('100.00' == percent || '100' == percent) {
				setTimeout(function () { triggerSuccess('Objects synchronized. Verify logs for confirmation.'); button.removeClass('wciz-btn__loader'); jQuery('#sync_products_initially').hide(); jQuery('.sync-products-initially').show(); }, 2000);
			}
			resultTab.html('<span class="wciz-product-initial-sync__progress-result" style="color: #3D8D7A;font-weight: 600;">Total synced : ' + sync_count + ' out of ' + out_of + '</span>');
			...
			form.addClass('is_hidden');
			resultBarWrap = form.next('.wciz-lists__progress');
			resultBar = resultBarWrap.find('.wciz-lists__progress-bar');
			resultTab = resultBarWrap.next('.wciz-sync-result');
			...
			if ('100.00' == percent || '100' == percent) {
				setTimeout(function () { triggerSuccess('Objects synchronized. Verify logs for confirmation.') }, 3000);
			}
			resultTab.html('<span class="wciz-lists__progress-result">Total synced : ' + sync_count + ' out of ' + out_of + '</span>');
		}
	}

	function startInitialSyncV2() {
		var event = 'get_available_object_feed_ids';
		jQuery.post(ajax_url, { action, nonce, event }).done(function (response) {
			response = JSON.parse(response);
			if (response.success) {
				if (response.feed_data.length) {
					startInitialSyncProcessV2(response.feed_data, 0);
				}
			} else {
				triggerError(response.message);
			}
	...
	function startInitialSyncProcessV2(feed_data, feed_step) {
		var current_feed = feed_data[feed_step];
		var feed_id = current_feed.feed_id;
		var feed_object = current_feed.object;
		var event = 'fetch_required_sync_count';
		var sync_type = 'bulk_data_sync';
		var sync_count = 0;
		jQuery.post(ajax_url, { action, event, nonce, feed_id, sync_type }).done(function (response) {
			response = JSON.parse(response);
			if (response.success) {
				syncInitialBulkDataV2(feed_id, sync_type, 0, feed_object, sync_count, feed_data, feed_step);
			} else {
				triggerError(response.message);
			}
		})
	...
	function syncInitialBulkDataV2(feed_id, sync_type, step, feed_object, sync_count, feed_data, feed_step) {
		const event = 'sync_bulk_data_request';
		console.log('object', feed_object, 'step', step);
		jQuery.post(ajax_url, { action, event, nonce, step, feed_id, sync_type }).done(function (response) {
			response = JSON.parse(response);
			if (response.success) {
				sync_count = sync_count + response.synced_data.length;
				updateInitialProgressBarV2(response.percent, feed_object, sync_count);
				if (response.has_next_step) {
					syncInitialBulkDataV2(feed_id, sync_type, response.next_step, feed_object, sync_count, feed_data, feed_step);
				}
			} else {
				feed_step++;
				if (feed_step < feed_data.length) {
					startInitialSyncProcessV2(feed_data, feed_step);
				} else {
					jQuery('#wciz-start-skip-setup').html('Next').show();
				}
			}
		});
	}

	function updateInitialProgressBarV2(percent, object, count) {

		resultBarWrap = jQuery('.wciz-product-initial-sync__progress');

		if (!resultBarWrap.hasClass('active')) {
			resultBarWrap.slideDown('400', function () {
				jQuery('html, body').animate({
					scrollTop: resultBarWrap.offset().top
				}, 2000, function () {
				});
				resultBarWrap.addClass('active');
			});
		}

		resultBar = resultBarWrap.find('.wciz-product-initial-sync__progress-bar');
		resultTab = resultBarWrap.find('.wciz-sync-result');
		resultBarWrap.show();
		resultBar.css('width', percent + '%');
		resultBar.html(percent + '%');
		...
		resultTab.html('<span class="wciz-product-initial-sync__progress-result">Total ' + object_name + ' synced : ' + count + '</span>');
	}


	/**
	 * Handle authorisation form according to app.
	 */
	$(document).on('change', '#wciz-zoho-use-global-app', function (e) {
		handle_auth_field();
	});

	handle_auth_field();

	function handle_auth_field() {

		var client_id_tr, secret_id_tr, job;
		if (true == $('#wciz-zoho-use-global-app').prop('checked')) {
			job = 'hide';
			$('#wciz-zoho-authorize-button').html(connect.login);
			var domain = jQuery('#wciz-zoho-domain').val();
			if ('ca' == domain) {
				jQuery('#wciz-zoho-domain').val('in');
			}
		} else {
			job = 'show';
			$('#wciz-zoho-authorize-button').html(connect.authorize);
		}

		$('form#wciz-zoho-auth-portal tr').each(function (index, value) {

			if ('wciz-zoho-secret-id' == $(this).find('td').find('input').attr('id')) {
				secret_id_tr = $(this);
			}

			if ('wciz-zoho-client-id' == $(this).find('td').find('input').attr('id')) {
				client_id_tr = $(this);
			}

			if ('wciz-zoho-redirect-uri' == $(this).find('td').find('input').attr('id')) {
				redirect_uri_tr = $(this);
			}

			if ('object' == typeof client_id_tr) {
				if ('hide' == job) {
					client_id_tr.hide();
				} else if ('show' == job) {
					client_id_tr.show();
			...
			if ('object' == typeof redirect_uri_tr) {
				if ('hide' == job) {
					redirect_uri_tr.hide();
				} else if ('show' == job) {
					redirect_uri_tr.show();
				}
			}
		});
	}
	/* Add or filter */
	$(document).on('click', '.condition-or-btn', function (e) {
		e.preventDefault();

		let object = $('#wciz-feeds-zoho-object').val();
		if ('-1' == object) {
			triggerError('Please select a object');
			return;
		}

		let nextOrIndex = $(this).attr('data-next-or-index');
		let nextOrHtml = createOrFilteredHtml(nextOrIndex);
		$(this).before(nextOrHtml);
		nextOrIndex++;
		$(this).attr('data-next-or-index', nextOrIndex);
	});

	/* Delete or filter */
	$(document).on('click', 'img.dashicons-trash', function (e) {
		let row = $(this).parents('.or-condition-filter');
		row.slideUp('400', function () {
			row.remove();
		});
	});
	/* Add and filter */
	$(document).on('click', '.condition-and-btn', function (e) {
		e.preventDefault();
		...
		let nextOrIndex = $(this).attr('data-or-index');
		let nextAndIndex = $(this).attr('data-next-and-index');
		let nextAndHtml = createAndFilteredHtml(nextAndIndex, nextOrIndex);

		$(this).before(nextAndHtml);
		nextAndIndex++;
		$(this).attr('data-next-and-index', nextAndIndex);
	});

	/* Delete and filter */
	$(document).on('click', '.dashicons-no', function (e) {
		let row = $(this).parents('.and-condition-filter');
		row.slideUp('400', function () {
			row.remove();
		});
	});
	...
	$(document).on('change', '.condition-option-field', function () {

		let condition = $(this).val();
		let value = $(this).next('.condition-value-field');
	...
	$('#wciz-zoho-enable-add-condition').on('change', function () {

		let enabled = $(this).is(':checked');
		if (enabled) {
			$('.wciz-initial-filter').find('.condition-value-field').attr('required', true);
		} else {
			$('.wciz-initial-filter').find('.condition-value-field').removeAttr('required');
		}
	});
