...
jQuery(document).ready(function () {
	...
	const adminUrl = mwb_cf7_integration_ajax_data.adminUrl;
	...
	const defaultFeedTitle = () => {
		...
		if (jQuery('body').hasClass('post-type-wciz_cf7_zoho_feeds')) {
	...
	const hideFeedEditOptions = () => {
		...
		if ('-1' == form) {
			jQuery('#mwb_zoho_feeds_feeds_condition_meta_box').slideUp();
		} else {
			jQuery('#mwb_zoho_feeds_feeds_condition_meta_box').slideDown();
		}
	...
	const display_tooltip = () => {
		...
		jQuery('.mwb_zoho_cf7_tips').tipTip(args);
	}
	...
	const initiliazeFeedForm = () => {
		...
		if ('-1' != object && '-1' != form && '' != post_id && 'wciz_cf7_zoho_feeds' == post_type) {
			if ('publish' == post_status) {
				...
				triggerInfo('This feed is in Sandbox mode, Publish it in order to see the feed mapping');
				jQuery('#mwb-feeds-' + crm + '-object').trigger('change');
			}
		}

		if (jQuery('body').hasClass('post-type-wciz_cf7_zoho_feeds')) {
			if (jQuery('#mwb-zoho-enable-add-condition').is(':checked')) {
	...
	const resetFeedForm = () => {
		...
		jQuery('.mwb_zoho_feeds__feeds-wrap').nextAll('.mwb-content-wrap').slideUp();
		jQuery('.mwb_zoho_feeds__feeds-wrap').nextAll('.mwb-content-wrap').remove();
		jQuery('#mwb_zoho_feeds_feeds_condition_meta_box').slideUp();
	}
	 ...
	 * @return   html.
	 */
	const createRecordImageField = (formFields) => {
		...
		if (jQuery('body').hasClass('post-type-wciz_cf7_zoho_feeds')) {
			...
			jQuery('#mwb-zoho-record-img').html(html);
		}
	...
	const manageAssociationFields = (objects = false) => {
		...
		if (jQuery('body').hasClass('post-type-wciz_cf7_zoho_feeds')) {
			...
			jQuery('#mwb_' + crm + '_cf7_feeds_add_owner_meta_box').css('display', 'none');
			...
			jQuery('#mwb_' + crm + '_cf7_feeds_add_layout_meta_box').css('display', 'none');
			jQuery('#mwb_' + crm + '_cf7_feeds_associate_campaign_meta_box').css('display', 'none');
			jQuery('#mwb_' + crm + '_cf7_feeds_add_lead_meta_box').css('display', 'none');
			jQuery('#mwb_' + crm + '_cf7_feeds_add_tag_meta_box').css('display', 'none');
			...
			jQuery('#mwb_' + crm + '_cf7_feeds_add_rimage_meta_box').css('display', 'none');

			//get dependent feeds.
			let dependent = jQuery('#wciz_cf7_dependent_feed').val();

			if (dependent.length > 0) {
				jQuery('#mwb_' + crm + '_cf7_feeds_condition_meta_box').css('display', 'none');
				jQuery('#mwb-' + crm + '-cf7-select-form option:not(:selected)').attr('disabled', 'disabled');
				...
				jQuery('#mwb_' + crm + '_cf7_feeds_add_attachment_meta_box').slideDown();


				if (objects == 'Contacts') {
					jQuery('#mwb_' + crm + '_cf7_feeds_associate_account_meta_box').slideDown();
					jQuery('#mwb_' + crm + '_cf7_feeds_add_rimage_meta_box').slideDown();
					jQuery('#mwb_' + crm + '_cf7_feeds_associate_campaign_meta_box').slideDown();
				}


				if (objects == 'Leads') {
					jQuery('#mwb_' + crm + '_cf7_feeds_add_lead_meta_box').slideDown();
					jQuery('#mwb_' + crm + '_cf7_feeds_add_rimage_meta_box').slideDown();
					jQuery('#mwb_' + crm + '_cf7_feeds_associate_campaign_meta_box').slideDown();
				}
				...
				if (objects == 'Accounts' || objects == 'Products' || objects == 'Vendors') {
					jQuery('#mwb_' + crm + '_cf7_feeds_add_rimage_meta_box').slideDown();
				}

				if (objects == 'Deals' || objects == 'Contacts') {
					jQuery('#mwb_' + crm + '_cf7_feeds_add_lead_meta_box').slideDown();
					jQuery('#mwb_' + crm + '_cf7_feeds_add_lead_meta_box').find('#mwb-wrap-lead-status').slideUp();
				}

				if ('Notes' !== objects) {
					jQuery('#mwb_' + crm + '_cf7_feeds_add_note_meta_box').slideDown();
					jQuery('#mwb_' + crm + '_cf7_feeds_add_tag_meta_box').slideDown();
					jQuery('#mwb_' + crm + '_cf7_feeds_add_owner_meta_box').slideDown();
					jQuery('#mwb_' + crm + '_cf7_feeds_add_layout_meta_box').slideDown();
				} else {
					jQuery('#mwb_' + crm + '_cf7_feeds_add_note_meta_box').slideUp();
				}

				if ('Leads' == objects) {
					jQuery('#mwb_' + crm + '_cf7_feeds_add_tag_meta_box').slideUp();
				}
	...
	const manageAuthForm = (method = false) => {
		...
		let form = jQuery('#mwb_cf7_integration_account_form');

		if (false == method) {
			method = jQuery('#mwb-zcf7-own-app').is(':checked');
		}
	...
	jQuery('input[name="mwb_setting[enable_notif]"]').on('change', function () {
		...
		if (email_enable) {
			jQuery('#mwb_zoho_cf7_email_notif').removeClass('is_hidden');

		} else {
			jQuery('#mwb_zoho_cf7_email_notif').addClass('is_hidden');
		}
	});
	...
	jQuery(document).on('click', '.trigger-setup-guide', function () {
		...
		let own_app = jQuery('#mwb-zcf7-own-app').is(':checked') ? 'yes' : 'no';
		let input = {
			action: action,
			nonce: nonce,
			event: event,
	...
	jQuery('#mwb-' + crm + '-cf7-authorize-button').on('click', function (e) {
		...
		let client_id = jQuery('#mwb-zcf7-client-id').val();
		let secret_id = jQuery('#mwb-zcf7-secret-id').val();
		let domain = jQuery('#mwb-zcf7-domain').val();
		let method = jQuery('#mwb-zcf7-own-app').is(':checked');

		if (method) {
			if (client_id.length == 0 || secret_id.length == 0) {
	...
	jQuery('#mwb-' + crm + '-cf7-select-form').on('change', function () {
		...
		result.then((response) => {
			if (true == response.status) {
				...
				createRecordImageField(formFields);
		...
		result.then((response) => {
			...
			if (true == response.status) {
				if (response.hasOwnProperty('data')) {
					if (true == response.data.success) {
						updateSelectOptions(select_id, response.data.objects);
						jQuery('.mwb_cf7_integration__feeds-wrap').nextAll('.mwb-content-wrap').slideUp();
						jQuery('.mwb_cf7_integration__feeds-wrap').nextAll('.mwb-content-wrap').remove();
					} else {
	...
	jQuery('#mwb-' + crm + '-refresh-fields').on('click', function () {
		...
		if ('-1' == module) {
			triggerError('Please select a ZOHO Object first');
			return;
		}
	...
	jQuery(document).on('click', '#add-new-field-btn', function (e) {
		...
		if ('-1' == object || '-1' == field) {
			triggerInfo('Select a valid ZOHO object and its corresponding field');
			return;
		}
	...
	jQuery(document).on('change', '.filter-feeds-by-form', function () {
		...
		result.then((response) => {
			if (true == response.status) {
				if (true == response.data.status) {
					jQuery('#mwb-' + crm + '-cf7_independent').slideUp();
					jQuery('#mwb-' + crm + '-cf7_independent').html(response.data.feeds);
					jQuery('#mwb-' + crm + '-cf7_independent').slideDown();
				}
			} else {
